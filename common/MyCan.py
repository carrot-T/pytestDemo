...
def periodic_send(frame_id,signals,channel=0):
    db_msg = db.get_message_by_frame_id(frame_id)
    msg = create_msg(db_msg,signals, channel)
    period = db_msg.cycle_time/1000
    task = hwhandle.send_periodic(msg,period)
...
def create_msg(db_msg,signals,channel=0):
    signal_dict = dict()
    ...
    # 信号初始值
    print('initial_signal：' + str(signal_dict))
    # 更新该字典键值对的值
    signal_dict.update(signals)
    print('update_signal：' + str(signal_dict))

    msg_data_encode = db_msg.encode(signal_dict)
    print('msg_data_encode：' + msg_data_encode.__str__())

    hex_str = binascii.hexlify(msg_data_encode).decode('utf-8')
    print('16进制显示：' + hex_str)

    msg_data_decode = db_msg.decode(msg_data_encode,decode_choices=False)
...
db_msg110 = db.get_message_by_frame_id(0x110)
print('msg_110:' + str(db_msg110))
signals110 = {'BDCS1_PowerManageMode': 0x6, 'BDCS1_PetToLookAfterRmd': 0x1}
create_msg(db_msg110,signals110,0)

db_msg243 = db.get_message_by_frame_id(0x243)
print('msg_243:' + str(db_msg243))
signals243 = {'AICS2_StopChargeing': 0x6, 'AICS2_OTAMode': 0x0}
create_msg(db_msg243,signals243,0)
